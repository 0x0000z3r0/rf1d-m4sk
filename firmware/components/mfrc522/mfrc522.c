...
// ESP8266 HSPI pins are fixed by hardware: SCK=GPIO14, MISO=GPIO12, MOSI=GPIO13.
// We use software CS so the chip-select can live on a safer GPIO.
...
static inline void mfrc522_cs_low(void)
{
    gpio_set_level(MFRC522_PIN_SS, 0);
}

static inline void mfrc522_cs_high(void)
{
...
static int mfrc522_to_card(uint8_t command, const uint8_t *send_data, uint8_t send_len,
...
static int mfrc522_request(uint8_t req_mode, uint8_t *tag_type)
{
    ...
    status = (uint8_t)mfrc522_to_card(PCD_TRANSCEIVE, tag_type, 1, tag_type, &back_bits);
...
static int mfrc522_anticoll(uint8_t *ser_num)
{
    ...
    ser_num[1] = 0x20;
    status = (uint8_t)mfrc522_to_card(PCD_TRANSCEIVE, ser_num, 2, ser_num, &back_bits);
...
esp_err_t mfrc522_init(void)
{
    ...
    gpio_config_t out_conf = {
        ...
        .pin_bit_mask = (1ULL << MFRC522_PIN_SS) |
                        (1ULL << MFRC522_PIN_RST),
    ...
    s_initialized = true;
    ESP_LOGI(TAG, "MFRC522 initialized (HSPI: SCK=14 MOSI=13 MISO=12 SS=%d RST=%d)",
...
{
    if (!s_initialized || uid == NULL || uid_len == NULL || *uid_len < MFRC522_MAX_UID_LEN) {
        return false;
    }
